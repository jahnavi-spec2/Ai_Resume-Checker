...
export const analyzeJobMatch= asyncHandler(async(req,res)=>{
    const {resumeId,verisonId,jobTitle,jobDescription}= req.body;
...
//GET/api/v1/jobMatching

export const getUserJobMatches=asyncHandler(async(req,res)=>{
    asyncHandller (async(req,res)=>{
        const jobMatches=await JobMatch.find({user:req.user._id})
        .populate("resume","title")
        .sort({createdAt:-1});

        return res.status(200).json(new ApiResponse(200,{jobMatches},"Job matches retrieved successfully"))
    })
});
...
user:req.user._id,
  ...
  return res.status(200).json(    new ApiResponse(200, { jobMatch }, "Job match details retrieved successfully")
  );
